    ...
    <div class="grid grid-cols-4 gap-x-10 gap-y-10 w-fit">
        @foreach ($notes as $note)
            ...
            <div class="w-[320px] h-[380px] rounded-[24px] p-5 relative"
                style="background-color: {{ $colors[$note->category?->color ?? 'blue'] }}">
                ...
                <div class="bg-[#F4F4F4] rounded-[18px] h-[285px] px-5 py-5 relative">
                    ...
                    <span
                        class="absolute bottom-4 right-5 text-[12px] text-gray-400">{{ $note->created_at->format('d/m/Y') }}</span>
                </div>
            </div>
        @endforeach
    </div>
            ...
            <form action="" id="editForm" method="POST" class="px-6 py-5 flex flex-col gap-4">
                ...
                <div class="pb-3">
                    ...
                    <textarea name="content" id="editContent" rows="5"
                        class="w-full px-4 py-2.5 rounded-xl border border-gray-200 bg-gray-50 text-sm text-gray-900">
                   </textarea>
                </div>
