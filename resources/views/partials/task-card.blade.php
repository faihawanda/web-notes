<div class="task-card bg-white rounded-[24px] border border-[#EDF0FA] shadow-sm p-5 mb-5">
        ...
        <div class="flex items-center gap-2">
            <form action="{{ route('tasks.updateStatus', $task->id) }}" method="POST" class="inline">
                ...
                <select name="status" onchange="this.form.submit()"
                    class="text-xs bg-gray-50 border border-gray-200 rounded-lg p-1 text-gray-600 outline-none cursor-pointer font-medium">
            ...
            <button onclick="openEditModal('{{ $task->id }}', '{{ addslashes($task->title) }}', '{{ addslashes($task->category) }}')"
                class="text-[#2F6BFF] text-xl hover:scale-110 duration-200">
    ...
    <div class="space-y-3">
            ...
            <div class="flex items-center justify-between group/item">
                <div class="flex items-center gap-2">
                    <form action="{{ route('subtasks.toggle', $subtask->id) }}" method="POST">
                        ...
                        @method('PATCH')
                        <button type="submit"
                            class="w-5 h-5 rounded-full border-2 border-[#2F6BFF] shrink-0 flex items-center justify-center transition duration-200 {{ $subtask->is_completed ? 'bg-[#2F6BFF]' : '' }}">
                    ...
                    <span
                        class="text-[17px] leading-none {{ $subtask->is_completed ? 'text-[#98A2B3] line-through' : 'text-[#707991]' }}">
                        {{ $subtask->text }}
                    </span>
                </div>
        ...
        <form action="{{ route('subtasks.store', $task->id) }}" method="POST"
            class="mt-4 flex items-center justify-between gap-2 border-t border-gray-100 pt-3 w-full min-w-0">
            @csrf
            <div class="flex-1 min-w-0">
                <input type="text" name="text" required placeholder="Add new item..."
    ...
    <div class="flex justify-between items-center mt-6">
        ...
        <form action="{{ route('tasks.destroy', $task->id) }}" method="POST"
            onsubmit="return confirm('Yakin mau hapus project ini beserta seluruh list di dalamnya?')">
            @csrf
            @method('DELETE')
            <button type="submit" class="text-[18px] hover:scale-120 duration-200 transition"
